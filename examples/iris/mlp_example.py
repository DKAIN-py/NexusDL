...
# print(f"x_test value: {x_test_raw} | x_test shape : {x_test_raw.shape}")
...
with no_grad():
    for i, data in enumerate(x_test_raw):
        ...
        if int(predicted_class.item()) == int(y_test_raw[i][0]):
            correct += 1
    data = np.array([5.5, 2.4, 3.8, 1.1])
    single_data = Nexus(data.reshape(1,-1))
    print(single_data)
    logits = model.forward(single_data)
    print(logits)
    prob = 1.0/(1.0 + np.exp(-logits.value))
    print(probs)
    print(f"single test results: {np.round(prob)}")
print(f"\nNumber of correct predictions: {correct} / {len(x_test_raw)}")
print(f"Accuracy: {(correct / len(x_test_raw)) * 100:.2f}%")


# model.export_weights("examples/iris")
